...
(defun LISP_TO_LI_atom_var (expr env)
  ...
  (if pos 
    (cons :var pos) 
    (warn "La variable ~s n'est pas connu dans l'environnement ~s" expr env))))
...
(defun LISP_TO_LI_noAtom_set-var (args env)
  (if (symbolp (first args)) 
    (list :set-var (cdr (LISP_TO_LI (first args) env)) (LISP_TO_LI (second args) env)) 
    (list* :setf (LISP_TO_LI (first args ) env))))
;(trace LISP_TO_LI_noAtom_set-var)
;==========================V ~
...
(defun LISP_TO_LI_noAtom_defun (args env)
  (list ':call 'set-defun (cons ':lit (first args)) (list ':lit ':lambda (length (second args)) (LISP_TO_LI (third args) env))))
...
;(trace LISP_TO_LI_noAtom_set-cvar)
;==========================X
...
;(trace LISP_TO_LI_noAtom_set-fun)
;==========================X
